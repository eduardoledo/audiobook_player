...
/// Probe ASR around a timestamp for the EPUB prologue opening phrase.
Future<void> main(List<String> args) async {
  if (args.length < 3) {
    stderr.writeln('Usage: dart run tool/probe_at.dart <audio> <epub> <seconds>');
    exit(64);
  }
  final audio = args[0];
  final epubPath = args[1];
  final center = double.parse(args[2]);

  final epub = parseEpubStructure(epubPath);
  final prologue = epub.entries.first;
  stdout.writeln('Prologue: «${prologue.firstWords}»');
  stdout.writeln('Needle: ${prologue.firstWordsNormalized.join(' ')}');
  ...
  final offsets = <double>[
    for (var d = -30.0; d <= 45.0; d += 5) center + d,
  ];
  final seen = <int>{};
  ...
  for (final t in offsets) {
    ...
    final pcm = await extractPcmWindow(
      ...
      fastSeek: false, // accurate seek for this probe
    );
    if (pcm == null) {
    ...
    stream.free();
    final depth =
        progressiveWordMatchCount(text, prologue.firstWordsNormalized);
    final mark = depth >= 5 ? ' <<<' : '';
    stdout.writeln(
      '${_fmt(t)}  depth=$depth/${prologue.firstWordsNormalized.length}  "$text"$mark',
    );
  }
